...
// =======================
// Image Path Resolver (CRITICAL FIX)
// This function determines the FINAL image source URL
// =======================
function resolveImagePath(imagePath) {
    // If the path is null, undefined, or empty, return a generic placeholder
    if (!imagePath) {
        return "https://via.placeholder.com/200x200?text=No+Image";
    }

    // If the path already starts with http/https, use it directly (external URL)
    if (imagePath.startsWith('http') || imagePath.startsWith('https')) {
        return imagePath;
    }

    // 🛑 FIX: If it's a local filename, prepend the folder name
    // This fixes the 404 error when images are in the 'images/' folder
...
// ... (omitted for brevity, assume original function is here)
// =======================
...
  let vendorCardsHTML = "";
  
  // 🟢 FIX: Use the path resolver function
  const imageSource = resolveImagePath(product.image);
...
// =======================
// Recommendation Card (FIXED IMAGE PATH)
// =======================
